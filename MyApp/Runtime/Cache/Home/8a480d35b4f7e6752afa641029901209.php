...
<form action="<?php echo U('modify');?>" method="post" class="js-ajax-form">
    <div class="layerCon bg_f">
        <input name="pt_id" type="hidden" value="<?php echo ($info["pt_id"]); ?>"/>
            ...
            <tr class="h50">
                ...
                <td>
                    <select id="pidname" onclick="modifypid(this)">
                        ...
                        <?php if(is_array($pidname2)): $i = 0; $__LIST__ = $pidname2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pidnames2): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pidnames2["pt_id"]); ?>"  onclick="modifypid(this)" <?php if(($pidnames2["pt_id"]) == $pid): ?>selected<?php endif; ?>><?php echo ($pidnames2["pt_receptionname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </td>
                <td>上级名称：</td>
                <td>
                    <select name="pt_superior" id="ptsuperior">
                        ...
                        <?php if(is_array($pid2)): $i = 0; $__LIST__ = $pid2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pids): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pids["pt_id"]); ?>" <?php if(($pids["pt_id"]) == $info["pt_superior"]): ?>selected<?php endif; ?>><?php echo ($pids["pt_receptionname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </td>
            </tr>
            <tr class="h50">
                <td>前台名称：</td>
                <td>
                    <input class="must" type="text" name="pt_receptionname" id="" value="<?php echo ($info["pt_receptionname"]); ?>" />
                </td>
                <td>标语：</td>
                <td>
                    <input class="must" type="text" name="pt_backstagename" id="" value="<?php echo ($info["pt_backstagename"]); ?>" />
                </td>
            </tr>
            <tr class="h50">
                <td>是否启用：</td>
                ...
                <td>排序：</td>
                <td>
                    <input class="must" type="text" name="pt_order" id="" value="<?php echo ($info["pt_order"]); ?>" />
                </td>
            </tr>
            <tr class="h50">
                <td>PC图片地址：</td>
                <td colspan="3">
                    <input type="text" name="pt_pcimg" id="" value="<?php echo ($info["pt_pcimg"]); ?>"  placeholder="请输入图片地址，如http://xxx.com/xxxx.png"/>
                </td>
            </tr>
            <tr class="h50">
                <td>添加时间：</td>
                <td colspan="3">
                    <input class="must" type="text" name="pt_time" id="" value="<?php echo ($info["pt_time"]); ?>" disabled />
                </td>
            </tr>
            <tr class="h50">
                <td>类型图片：</td>
                <td>
                    <label style="display:inline-block;">
                        <input type="radio" value="2"   style="width: 15px;height: 15px;" name="pt_img1"  onclick="displaytypes()" <?php if($info["pt_color"] != NULL): ?>checked<?php endif; ?>/>
                        <a  style="display: inline-block !important;" onclick="displaytypes(this)">填写图标地址</a></label>
                    <label style="display:inline-block;">
                    <input type="radio" value="1"   style="width: 15px;height: 15px;" name="pt_img1"  onclick="displaytypes()" <?php if($info["pt_color"] == NULL): ?>checked<?php endif; ?>/>
        ...
        <div  class="search-form-div" style="display: none;">
            <table class="layerTable" border="0">
                <tr class="h50">
                    <td>图标地址：</td>
                    <td>
                        <input class="must" type="text" name="pt_img" id="ptimg" value='<?php echo ($info["pt_img"]); ?>' />
                    </td>
                </tr>
                <tr class="h50">
                    <td>图标颜色：</td>
                    <td><input type="text" name="pt_color" value='<?php echo ($info["pt_color"]); ?>'  class="ptcolor" id="nowColor" />
                    ...
                    <tr class="h50">
                        ...
                        <td>
                            ...
                            <div class="imgadd" id="u_favicon">
                               <?php if(!empty($info["pt_img"])): ?><input name='pt_img' type='hidden' id="ptimgs" value='<?php echo ($info["pt_img"]); ?>'>
                                    <img src='<?php echo ($info["pt_img"]); ?>'   width='80px;' height='80px;'  style='margin-top:-80px;'>
                                    <img src='/Public/img/trash.png' class='delthis' style='margin-right:2px;'>
                                   <input name='pt_color' id='pt_color' type='hidden' value=''><?php endif; ?>
                            </div>
                        </td>
                    </tr>
                </table>
