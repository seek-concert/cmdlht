...
<div class="toolsBar">
    ...
    <ul>
        ...
        <form action="<?php echo U('servicedemand/excel_order');?>" method="post">
...
<div class="tableCon">
    <div style="visibility: hidden;display: none;">
        <div id="search-form-div">
            <form action="/Systemorders/index" method="post" id="search-form" class="layui-form">
    ...
    <form action="<?php echo U('sort');?>" method="post" class="js-ajax-form" id="js-ajax-form">
        <table class="table" border="0">
            ...
            <?php if(is_array($servicedemand_info)): $i = 0; $__LIST__ = $servicedemand_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php if(($info["sd_tjtype"]) == "10000"): ?>直接购买<?php endif; ?>
                        <?php if(($info["sd_tjtype"]) == "20000"): ?>提交需求<?php endif; ?>
                        <?php if(($info["sd_tjtype"]) == "30000"): ?>上门评估<?php endif; ?></td>
                    <td><?php if(($info["sd_tjtype"]) == "10000"): ?><a class="table_btn button1" style="background-color: #00a0e9 !important;" title="详细" onclick="layerPage('施工需求详情','<?php echo U('modify',array('sd_id'=>$info['sd_id'],));?>','700px','580px')">施工需求详情</a><?php endif; ?>
                        <?php if(($info["sd_tjtype"]) == "20000"): if($info['sd_pguid'] != 0): if(in_array($info['sd_id'],$csdids) || $info['sd_pguid'] != 0): ?><a class="table_btn button1" style="background-color: #00a0e9 !important;" title="详细" onclick="layerPage('施工需求详情','<?php echo U('detail',array('sd_id'=>$info['sd_id']));?>','700px','580px')">施工需求详情</a>
                                    <?php else: ?>
                                         <a class="table_btn button1" style="background-color: #fd2b37 !important;"  title="添加" onclick="layerPage('添加施工需求','<?php echo U('servicedemand/insert',array('sd_id'=>$info['sd_id']));?>','800px','575px')"><span>添加施工需求</span></a><?php endif; ?>
                                <?php else: ?>
                        ...
                        <?php if(($info["sd_tjtype"]) == "30000"): if($info['sd_pguid'] != 0): if(in_array($info['sd_id'],$csdids)): ?><a class="table_btn button1" style="background-color: #00a0e9 !important;" title="详细" onclick="layerPage('施工需求详情','<?php echo U('servicedemand/detail',array('sd_id'=>$info['sd_id']));?>','700px','580px')">施工需求详情</a>
                                    <?php else: ?>
                                    <a class="table_btn button1" style="background-color: #fd2b37 !important;"  title="添加" onclick="layerPage('添加施工需求','<?php echo U('servicedemand/insert',array('sd_id'=>$info['sd_id']));?>','800px','575px')"><span>添加施工需求</span></a><?php endif; ?>
                                <?php else: ?>
                                <a class="table_btn button1" style="background-color: #0a6999 !important;"  title="添加" onclick="layerPage('添加上门评估','<?php echo U('modifys_insert',array('sd_id'=>$info['sd_id']));?>','750px','480px')"><span>添加上门评估</span></a><?php endif; endif; ?>
