...
function layer_showtrue(title, url) {
    if (title == null || title == '') {
        title = false;
    };
    if (url == null || url == '') {
...
function viewshow(result) {
    var index = layer.load({ shade: [0.5, '#fff'] });
    $.ajax({
        type: "post",
        url: "/command/viewshow",
        data: { result: result },
        dataType: "json",
        success: function (e) {
            layer.msg(e.message, { icon: 1, time: 1000 });
            if (e.state) setTimeout(function () { location.replace(location.href) }, 1000);
        }
    })
}
function loading(e,title) {
    var data = new Array();
    for (var i = 0; i < e.ob.daycount.length; i++) {
        data[i] = [Date.UTC(e.ob.year, e.ob.month - 1, e.ob.daycount[i].day), e.ob.daycount[i].value];
    }
    $('#container').highcharts({
        ...
        title: {
            text:  e.ob.year + '年' + e.ob.month + '月 '+title
        },
        xAxis: {
            type: 'datetime'
        },
